...
The release relationship is explicit: Phase 5 may produce and, under Gate F,
publish an `Experimental` prerelease after all applicable packaging,
lifecycle, integrity, enrollment, and adversarial prerequisites pass. Existing
receiver-relative evidence supports only its stated lesser scope. Packaging or
publication never creates or preserves final `Qualified` status. Calibrated
review uses that exact frozen packaged candidate; its results enter a newly
reviewed final manifest and separately reviewed immutable release identity.
If calibrated work changes behavior/source, overlays, UAPI, timing, package or
generated bytes, signing, compatibility policy, or lifecycle tooling, assign a
new candidate and repeat every affected Phase 5 lifecycle and release check.
See `release/calibrated-review-release-policy-v1.json` and Decision 0010.
