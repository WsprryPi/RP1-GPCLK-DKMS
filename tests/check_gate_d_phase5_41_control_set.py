...
commit="640877c1f29297e2f6ea855742605550781256e9"
def frozen_payload(relative,expected):
 ...
 return payload
route=load("release/gate-d-route-compatibility-decision-phase5.41-v1.json")
plan=load("release/gate-d-target-operation-plan-phase5.41-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.41-v1.json")
instance=load("release/gate-d-execution-instance-phase5.41-v1.json")
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.41-v1.json")
index=load("release/gate-d-attempts-phase5.41-v1/index.json")
identity=load("docs/evidence/gate-d-phase5.41-qualification-install-identity.json")
inventory=load("docs/evidence/gate-d-phase5.41-predecessor-package-inventory.json")
release_inventory=load("docs/evidence/gate-c-phase5.41-release-input-inventory.json")
build=load("release/gate-c-representative-build-manifest-phase5.41-v1.json")
...
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.41-v1.json")
...
assert build["result"]["releaseInputInventory"]["sha256"]==sha(ROOT/"docs/evidence/gate-c-phase5.41-release-input-inventory.json")
release_inputs={pathlib.PurePosixPath(x["path"]).name:x["sha256"] for x in envelope["releaseInputs"]}
assert release_inputs==artifacts
assert route["candidate"]["archiveSha256"]==artifacts["rp1-gpclk-dkms-0.0.0-phase5.41.tar.gz"]
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.41-v1"; documents=[]
...
assert inputs[envelope["qualificationIdentity"]["path"]]==sha(ROOT/"docs/evidence/gate-d-phase5.41-qualification-install-identity.json")
with tempfile.TemporaryDirectory() as temporary:
...
for mutate in (lambda v:v["installedPackagePaths"].pop(),lambda v:v["installedPackagePaths"][0].update(type="symlink"),lambda v:v.update(packagePathsSha256="0"*64),lambda v:v["priorTerminalState"].update(liveOutput=True),lambda v:v["safety"].update(gpioAccess=True)):
 ...
 else: raise AssertionError("unsafe Phase 5.41 envelope accepted")
subprocess.run([sys.executable,str(ROOT/"scripts/generate_phase5_41_control_set.py"),"--check"],check=True)
print("Gate D Phase 5.41 offline control set: PASS")
