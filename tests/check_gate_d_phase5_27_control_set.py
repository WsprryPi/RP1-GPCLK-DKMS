...
"""Validate the sealed Phase 5.27 Gate D control set entirely offline."""
...
COMMIT="bfb92725631748db3f7f7def8d331442872cab7d"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.27-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.27-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.27-v1.json")
plan=load("release/gate-d-target-operation-plan-phase5.27-v1.json")
instance=load("release/gate-d-execution-instance-phase5.27-v1.json")
index=load("release/gate-d-attempts-phase5.27-v1/index.json")
assert gate_d_preroot.validate(envelope)=={"valid":True,"readOnly":True,"outputDisabled":True}
assert gate_d_root.validate(instance["qualificationRoot"],verify=False)==pathlib.Path("/home/pi/gate-d-qualification/phase5.27-bfb927256317")
assert bootstrap["qualificationRoot"]==plan["qualificationRoot"]==index["qualificationRoot"]==instance["qualificationRoot"]
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.27"
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.27-v1.json")
assert route["candidate"]["sourceCommit"]==COMMIT
assert route["candidate"]["archiveSha256"]=="c623a8ebf6b5dc01a6e85a17e8709c479ad349aa2a08b34a86d71a2dc2a6adbb"
assert route["evidence"]["moduleSha256"]=="0d0401ce932ca2b5020cce20e6cafbd8ee8d3133f8046ec12c8dc53a1e0541d6"
...
assert sum(x.startswith("release/gate-d-attempts-phase5.27-v1/gd-") for x in destinations)==38
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.27-v1"; documents=[]
...
with tempfile.TemporaryDirectory() as temporary:
 ...
 try:
  ...
  else: raise AssertionError("execution-ready instance accepted without authorization")
...
print("Gate D Phase 5.27 offline control set: PASS")
