...
python3 "$repo_dir/tests/check_phase4c_gpio20_gate.py"
...
if [ "$(uname -s)" = Linux ]; then
	...
	${CC:-cc} -std=c11 -Wall -Wextra -Werror \
		-I"$repo_dir/include/uapi" "$repo_dir/tests/phase4c_live_client.c" \
		-lm -o "$tmp_dir/phase4c_live_client"
	echo "Phase 4C live client compile: PASS"
else
