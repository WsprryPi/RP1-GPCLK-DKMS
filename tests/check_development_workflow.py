...
def test_render() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        result = command(str(source / "scripts/render-development-tree"), "--source", str(source), "--output", str(output), "--module-version", "1.1.2", cwd=source)
        ...
        assert 'PACKAGE_VERSION="1.1.2"' in (output / "dkms.conf").read_text()
        ...
        again = command(str(source / "scripts/render-development-tree"), "--source", str(source), "--output", str(output), "--module-version", "1.1.2", cwd=source)
        assert again.returncode == 2 and "already exists" in again.stderr
        (source / "README.md").write_text((source / "README.md").read_text() + "\ndirty\n")
        rejected = command(str(source / "scripts/render-development-tree"), "--source", str(source), "--output", str(base / "dirty"), "--module-version", "1.1.2", cwd=source)
        assert rejected.returncode == 2 and "dirty" in rejected.stderr
        allowed = command(str(source / "scripts/render-development-tree"), "--source", str(source), "--output", str(base / "allowed"), "--module-version", "1.1.2", "--allow-dirty", cwd=source)
...
def test_forbidden_transform() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        result = command(str(source / "scripts/render-development-tree"), "--source", str(source), "--output", str(base / "output"), "--module-version", "1.1.2", cwd=source)
...
case "$1" in
 ...
 status) printf 'rp1-gpclk-dkms/1.1.2, %s, installed\\n' "$RP1_TEST_KERNEL" ;;
esac
''')
        executable(tools/"modinfo", '''
field=$2
case "$field" in name) echo rp1_gpclk_dkms;; version) echo 1.1.2;; vermagic) echo "$RP1_TEST_KERNEL SMP mod_unload aarch64";; signer|sig_key|sig_hashalgo) :;; esac
''')
...
echo 1.1.2 >"$RP1_GPCLK_DEVELOPMENT_ROOT/sys/module/rp1_gpclk_dkms/version"
...
''')
        ...
        result=command(str(source/"scripts/development-install"),"--source",str(source),"--kernel",kernel,"--module-version","1.1.2","--route","gpio4","--live-output","0","--load","--evidence-directory",str(evidence),cwd=source,env=environment)
        assert result.returncode==0,result.stderr
        ...
        replaced=command(str(source/"scripts/development-install"),"--source",str(source),"--kernel",kernel,"--module-version","1.1.2","--route","gpio4","--live-output","0","--load","--evidence-directory",str(replacement),cwd=source,env=environment)
        ...
        assert not (fake_root/f"usr/src/rp1-gpclk-dkms-1.1.2").exists()
