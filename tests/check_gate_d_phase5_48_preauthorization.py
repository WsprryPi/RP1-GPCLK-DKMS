...
"""Validate the Phase 5.48 preauthorization recapture attestation."""
import hashlib
import json
import subprocess
...
assert value["controlSetCommit"] == "833db92a5b3aadf30c3dd617bea734d0d7f5b20a"
...
for key, relative in {
    "envelopeSha256": "release/gate-d-pre-root-bootstrap-envelope-phase5.48-v1.json",
    "executionInstanceSha256": "release/gate-d-execution-instance-phase5.48-v1.json",
    "attemptIndexSha256": "release/gate-d-attempts-phase5.48-v1/index.json",
}.items():
...
instance = json.loads(subprocess.check_output([
    "git", "show",
    f"{value['controlSetCommit']}:release/gate-d-execution-instance-phase5.48-v1.json"
], cwd=ROOT))
assert instance["authorization"]["targetExecutionApproved"] is False
assert instance["executionReady"] is False
print("Phase 5.48 preauthorization recapture: PASS")
