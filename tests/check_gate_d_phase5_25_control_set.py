...
plan=load("release/gate-d-target-operation-plan-phase5.25-v1.json")
instance=load("release/gate-d-execution-instance-phase5.25-v1.json")
index=load("release/gate-d-attempts-phase5.25-v1/index.json")
assert gate_d_preroot.validate(envelope)=={"valid":True,"readOnly":True,"outputDisabled":True}
assert gate_d_root.validate(instance["qualificationRoot"],verify=False)==pathlib.Path("/home/pi/gate-d-qualification/phase5.25-d9f8fd8b17f1")
assert bootstrap["qualificationRoot"]==plan["qualificationRoot"]==index["qualificationRoot"]==instance["qualificationRoot"]
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.25"
...
assert sum(x.startswith("release/gate-d-attempts-phase5.25-v1/gd-") for x in destinations)==38
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.25-v1"; documents=[]
...
print("Gate D Phase 5.25 offline control set: PASS")
