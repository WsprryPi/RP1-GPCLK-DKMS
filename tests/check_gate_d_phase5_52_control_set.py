...
"""Independently validate the complete Phase 5.52 Gate D control set."""
...
def frozen_payload(relative: str, expected: str) -> bytes:
    ...
    payload = subprocess.check_output([
        "git", "show", f"f710554c4697d75210cbd33c9eea13474d60557a:{relative}"
    ], cwd=ROOT)
...
snapshot_path = ROOT / "docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.52-v1.json"
snapshot = load("docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.52-v1.json")
build = load("release/gate-c-representative-build-manifest-phase5.52-v1.json")
route = load("release/gate-d-route-compatibility-decision-phase5.52-v1.json")
plan = load("release/gate-d-target-operation-plan-phase5.52-v1.json")
bootstrap = load("release/gate-d-qualification-bootstrap-plan-phase5.52-v1.json")
instance = load("release/gate-d-execution-instance-phase5.52-v1.json")
envelope = load("release/gate-d-pre-root-bootstrap-envelope-phase5.52-v1.json")
identity = load("docs/evidence/gate-d-phase5.52-qualification-install-identity.json")
inventory = load("docs/evidence/gate-d-phase5.52-predecessor-package-inventory.json")
index = load("release/gate-d-attempts-phase5.52-v1/index.json")

assert sha(snapshot_path) == "449201a0a51ca8b278b7ae077410e515aa9c176eac42f4ba86bd62ef4c36451f"
...
assert snapshot["administratorLedger"]["release"] == "0.0.0-phase5.51"
assert snapshot["administratorLedger"]["sha256"] == \
    "9683e2cb97f6d302bdcab74ec2a33748581302fc2023f178b7511edf437c884a"
assert snapshot["terminalRecovery"]["sha256"] == \
    build["target"]["terminalRecoveryJournalSha256"] == \
...
assert instance["executionPolicy"]["attemptPathNamespace"] == "phase5.52-f710554c4697"
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    rehearsal_instance_path = rehearsal_root / "release/gate-d-execution-instance-phase5.52-v1.json"
...
attempt_dir = ROOT / "release/gate-d-attempts-phase5.52-v1"
...
assert {path.name for path in (ROOT / "release").glob("gate-d-*-phase5.52-v1.json")} == {
    "gate-d-execution-instance-phase5.52-v1.json",
    "gate-d-pre-root-bootstrap-envelope-phase5.52-v1.json",
    "gate-d-qualification-bootstrap-plan-phase5.52-v1.json",
    "gate-d-route-compatibility-decision-phase5.52-v1.json",
    "gate-d-target-operation-plan-phase5.52-v1.json",
}
documents = []
...
for record in index["attempts"]:
    ...
    namespace = "/gate-d/runs/phase5.52-f710554c4697/"
...
for phase in ("phase5.42", "phase5.43", "phase5.45", "phase5.46", "phase5.47", "phase5.48", "phase5.50", "phase5.51"):
...
subprocess.run([sys.executable, str(ROOT / "scripts/generate_phase5_52_control_set.py"),
                "--check"], check=True)
print("Gate D Phase 5.52 offline control set: PASS")
