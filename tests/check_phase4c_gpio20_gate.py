...
"""Enforce the exact Phase 4C GPIO20 live-enrollment boundary."""
...
for token in (
    ...
    if token not in MAIN:
        raise SystemExit(f"exact GPIO20 enrollment gate missing {token}")
...
for token in (
    ...
    if token not in DISPATCH:
        raise SystemExit(f"truthful Phase 4C query/submit gate missing {token}")
...
for token in (
    ...
    if token not in EXECUTION:
        raise SystemExit(f"Phase 4C audit/restoration evidence missing {token}")

print("Phase 4C GPIO20 exact enrollment and restoration boundary: PASS")
