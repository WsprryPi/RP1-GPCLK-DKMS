...
"""Validate the sealed Phase 5.30 Gate D control set entirely offline."""
...
COMMIT="73f3f7df4d7df194a52d14cacf127d29a316545f"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.30-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.30-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.30-v1.json")
plan=load("release/gate-d-target-operation-plan-phase5.30-v1.json")
instance=load("release/gate-d-execution-instance-phase5.30-v1.json")
index=load("release/gate-d-attempts-phase5.30-v1/index.json")
assert gate_d_preroot.validate(envelope)=={"valid":True,"readOnly":True,"outputDisabled":True}
assert gate_d_root.validate(instance["qualificationRoot"],verify=False)==pathlib.Path("/home/pi/gate-d-qualification/phase5.30-73f3f7df4d7d")
assert bootstrap["qualificationRoot"]==plan["qualificationRoot"]==index["qualificationRoot"]==instance["qualificationRoot"]
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.30"
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.30-v1.json")
assert route["candidate"]["sourceCommit"]==COMMIT
assert route["candidate"]["archiveSha256"]=="ffc6b2e08ef8ed28550980c64bdc5c48e7d917c39e927f192e62c0451b408fd9"
assert route["evidence"]["moduleSha256"]=="2203aeea52b17c0f6ce233f55ddff1069c93e05294238f517078029c87d9e0a0"
...
assert sum(x.startswith("release/gate-d-attempts-phase5.30-v1/gd-") for x in destinations)==38
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.30-v1"; documents=[]
...
with tempfile.TemporaryDirectory() as temporary:
 ...
 try:
  ...
  result=gate_d_instance.validate(instance); assert result["inputsReady"] is True and result["executionReady"] is False and len(result["deferredRows"])==5
  try: gate_d_instance.validate(instance,require_ready=True)
  except ValueError: pass
  else: raise AssertionError("unauthorized instance accepted as execution-ready")
  bad=copy.deepcopy(instance); bad["authorization"]["targetExecutionApproved"]=True; bad["executionReady"]=True
  assert hashlib.sha256((json.dumps(bad,indent=2,sort_keys=True)+"\n").encode()).hexdigest()!=sources["/home/pi/gate-d-inputs/phase5.30-73f3f7df4d7d/control-set/release/gate-d-execution-instance-phase5.30-v1.json"]["sha256"]
...
print("Gate D Phase 5.30 offline control set: PASS")
