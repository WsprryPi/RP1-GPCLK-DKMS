...
"""Independently validate the complete Phase 5.46 Gate D control set."""
...
def frozen_payload(relative: str, expected: str) -> bytes:
    ...
    payload = subprocess.check_output([
        "git", "show", f"b43e2744b212f5bc53ad40584254f52310af4684:{relative}"
    ], cwd=ROOT)
...
snapshot_path = ROOT / "docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.46-v1.json"
snapshot = load("docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.46-v1.json")
build = load("release/gate-c-representative-build-manifest-phase5.46-v1.json")
route = load("release/gate-d-route-compatibility-decision-phase5.46-v1.json")
plan = load("release/gate-d-target-operation-plan-phase5.46-v1.json")
bootstrap = load("release/gate-d-qualification-bootstrap-plan-phase5.46-v1.json")
instance = load("release/gate-d-execution-instance-phase5.46-v1.json")
envelope = load("release/gate-d-pre-root-bootstrap-envelope-phase5.46-v1.json")
identity = load("docs/evidence/gate-d-phase5.46-qualification-install-identity.json")
inventory = load("docs/evidence/gate-d-phase5.46-predecessor-package-inventory.json")
index = load("release/gate-d-attempts-phase5.46-v1/index.json")

assert sha(snapshot_path) == "bc4c307350d6e74c9cbb85ef890fbaf0e8ad969ecdeb661a98703b70bd4a1859"
...
assert instance["executionPolicy"]["attemptPathNamespace"] == "phase5.46-b43e2744b212"
...
attempt_dir = ROOT / "release/gate-d-attempts-phase5.46-v1"
...
for record in index["attempts"]:
    ...
    namespace = "/gate-d/runs/phase5.46-b43e2744b212/"
...
for phase in ("phase5.42", "phase5.43", "phase5.45"):
...
subprocess.run([sys.executable, str(ROOT / "scripts/gate_d_live_snapshot_validate.py"),
                ...
                str(ROOT / "release/gate-d-pre-root-bootstrap-envelope-phase5.46-v1.json"),
                "--inventory", str(ROOT / "docs/evidence/gate-d-phase5.46-predecessor-package-inventory.json"),
                "--route", str(ROOT / "release/gate-d-route-compatibility-decision-phase5.46-v1.json"),
                "--build", str(ROOT / "release/gate-c-representative-build-manifest-phase5.46-v1.json")],
               check=True)
subprocess.run([sys.executable, str(ROOT / "scripts/generate_phase5_46_control_set.py"),
                "--check"], check=True)
print("Gate D Phase 5.46 offline control set: PASS")
