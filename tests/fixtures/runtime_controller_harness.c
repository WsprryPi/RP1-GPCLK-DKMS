...
static void reset(void)
{
 ...
 copyout_error = 0; foreign = false; allowed = true; route_lock = 0;
...
int main(void)
{
 ...
 assert(rp1_route_consumer_attach(false) == -EBUSY); route_lock = 0;
 ...
 reset(); value = request(1, 1); assert(call(&value) == 0);
 assert(rp1_route_consumer_attach(false) == 0); rp1_route_consumer_detach(true);
