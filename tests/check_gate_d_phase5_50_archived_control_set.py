...
"""Validate Phase 5.50 controls with only exact frozen archived tool bytes."""
...
ARCHIVE_SHA256 = "ef989bf79faa5c30ddaf8ac5651d75b1755ba2fa385680692183e6145b2927c2"
RELEASE = "0.0.0-phase5.50"

if len(sys.argv) != 2:
    raise SystemExit("usage: check_gate_d_phase5_50_archived_control_set.py ARCHIVE")
...
if archive.is_symlink() or not archive.is_file() or sha(archive) != ARCHIVE_SHA256:
    raise SystemExit("Phase 5.50 archive identity differs")

instance = json.loads((ROOT / "release/gate-d-execution-instance-phase5.50-v1.json").read_text())
plan = json.loads((ROOT / "release/gate-d-target-operation-plan-phase5.50-v1.json").read_text())
envelope = json.loads((ROOT / "release/gate-d-pre-root-bootstrap-envelope-phase5.50-v1.json").read_text())
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    with tarfile.open(archive, "r:gz") as source:
        ...
        for member in members:
            ...
            if (not pure.parts or pure.parts[0] != prefix or pure.is_absolute() or
                ...
                raise SystemExit("unsafe Phase 5.50 archive member")
    ...
    subprocess.run([
        ...
        str(ROOT / "release/gate-d-execution-instance-phase5.50-v1.json"),
    ], check=True)
...
print("Phase 5.50 exact archived control-set validation: PASS")
