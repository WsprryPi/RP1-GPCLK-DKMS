...
"""Validate the sealed Phase 5.35 Gate D control set entirely offline."""
...
COMMIT="23efb65ea749dc09eb0cbadc18074be83f4035a9"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.35-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.35-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.35-v1.json")
plan=load("release/gate-d-target-operation-plan-phase5.35-v1.json")
instance=load("release/gate-d-execution-instance-phase5.35-v1.json")
index=load("release/gate-d-attempts-phase5.35-v1/index.json")
...
assert gate_d_root.validate(instance["qualificationRoot"],verify=False)==pathlib.Path("/home/pi/gate-d-qualification/phase5.35-23efb65ea749")
assert bootstrap["qualificationRoot"]==plan["qualificationRoot"]==index["qualificationRoot"]==instance["qualificationRoot"]
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.35"
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.35-v1.json")
assert route["candidate"]["sourceCommit"]==COMMIT
assert route["candidate"]["archiveSha256"]=="247f954541abb25bce9a1b60841122eaf02985b9fcea37df7d32e12fe9bf6e4c"
assert route["evidence"]["moduleSha256"]=="937357896407e941d561e4cda1cbcfdcd0d4d986fed8ec2ac6d0be460c6fa46c"
...
assert sum(x.startswith("release/gate-d-attempts-phase5.35-v1/gd-") for x in destinations)==38
...
identity=load("docs/evidence/gate-d-phase5.35-qualification-install-identity.json")
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.35-v1"; documents=[]
...
with tempfile.TemporaryDirectory() as temporary:
 ...
 try:
  ...
  else: raise AssertionError("unauthorized Phase 5.35 instance accepted as execution-ready")
  ...
  assert hashlib.sha256((json.dumps(bad,indent=2,sort_keys=True)+"\n").encode()).hexdigest()!=sources["/home/pi/gate-d-inputs/phase5.35-23efb65ea749/control-set/release/gate-d-execution-instance-phase5.35-v1.json"]["sha256"]
...
print("Gate D Phase 5.35 offline control set: PASS")
