...
"""Validate the Phase 5.46 preauthorization recapture attestation."""
import hashlib
import json
import subprocess
...
assert value["controlSetCommit"] == "f1e5fa27bed175533f6a291152fa70700b88285b"
...
for key, relative in {
    "envelopeSha256": "release/gate-d-pre-root-bootstrap-envelope-phase5.46-v1.json",
    "executionInstanceSha256": "release/gate-d-execution-instance-phase5.46-v1.json",
    "attemptIndexSha256": "release/gate-d-attempts-phase5.46-v1/index.json",
}.items():
...
instance = json.loads(subprocess.check_output([
    "git", "show",
    f"{value['controlSetCommit']}:release/gate-d-execution-instance-phase5.46-v1.json"
], cwd=ROOT))
assert instance["authorization"]["targetExecutionApproved"] is False
assert instance["executionReady"] is False
print("Phase 5.46 preauthorization recapture: PASS")
