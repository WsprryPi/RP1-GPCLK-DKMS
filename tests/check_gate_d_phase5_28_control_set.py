...
"""Validate the sealed Phase 5.28 Gate D control set entirely offline."""
...
COMMIT="9c408ec493ab3fc1aa58142592b427faa96c6b4a"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.28-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.28-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.28-v1.json")
plan=load("release/gate-d-target-operation-plan-phase5.28-v1.json")
instance=load("release/gate-d-execution-instance-phase5.28-v1.json")
index=load("release/gate-d-attempts-phase5.28-v1/index.json")
assert gate_d_preroot.validate(envelope)=={"valid":True,"readOnly":True,"outputDisabled":True}
assert gate_d_root.validate(instance["qualificationRoot"],verify=False)==pathlib.Path("/home/pi/gate-d-qualification/phase5.28-9c408ec493ab")
assert bootstrap["qualificationRoot"]==plan["qualificationRoot"]==index["qualificationRoot"]==instance["qualificationRoot"]
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.28"
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.28-v1.json")
...
assert sum(x.startswith("release/gate-d-attempts-phase5.28-v1/gd-") for x in destinations)==38
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.28-v1"; documents=[]
...
print("Gate D Phase 5.28 offline control set: PASS")
