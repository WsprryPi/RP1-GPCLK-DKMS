...
work=$(mktemp -d /tmp/rp1-gpclk-phase4d-live.XXXXXX)
...
{
	set +e
	pkill -f "$work/phase4d_sdr_capture" 2>/dev/null
...
cc -std=c11 -Wall -Wextra -Werror -I"$source_dir/include/uapi" \
	"$source_dir/tests/phase4d_live_client.c" -lm -o "$work/phase4d_live_client"
g++ -std=c++17 -Wall -Wextra -Werror "$source_dir/tests/phase4d_sdr_capture.cpp" \
	$(pkg-config --cflags --libs SoapySDR) -o "$work/phase4d_sdr_capture"

openssl req -new -x509 -newkey rsa:2048 -nodes -days 1 \
	-subj /CN=RP1-GPCLK-Phase4D-Combined/ -keyout "$work/signing-key.pem" \
...
run_capture()
{
	...
	before=$(dmesg | wc -l)
	"$work/phase4d_sdr_capture" "$evidence_dir/$name.cf32" "$seconds" \
...
run_capture wspr wspr 115
python3 "$source_dir/tests/phase4d_analyze.py" "$evidence_dir" \
...
[[ $(grep -c 'phase4d generation=' "$evidence_dir/dmesg-live-delta.txt") -eq 14 ]]
! grep -Eq 'BUG:|WARNING:|Oops:|Call trace:|phase4d cleanup:|cleanup=-[1-9]' \
	"$evidence_dir/dmesg-live-delta.txt"
