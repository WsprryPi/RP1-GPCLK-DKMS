...
assert model["release"] == layout["release"] == "0.0.0-phase5.51"
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    assert (target / "usr/share/rp1-gpclk-dkms/0.0.0-phase5.51/overlay-contract-v1.json").is_file()
    assert (target / "usr/share/rp1-gpclk-dkms/0.0.0-phase5.51/permissions-enrollment-policy-v1.json").is_file()
    assert (target / "usr/share/rp1-gpclk-dkms/0.0.0-phase5.51/diagnostics-contract-v1.json").is_file()
    assert (target / "usr/share/rp1-gpclk-dkms/0.0.0-phase5.51/lifecycle-removal-contract-v1.json").is_file()
    assert (target / "usr/share/rp1-gpclk-dkms/0.0.0-phase5.51/gate-d-phase5.24-residue-recovery-v1.json").is_file()
    assert (target / "usr/libexec/rp1-gpclk-dkms/lifecycle-policy").is_file()
    assert not (target / "usr/share/rp1-gpclk-dkms/0.0.0-phase5.51/gate-d-execution-instance-v1.json").exists()
    assert (target / "usr/share/rp1-gpclk-dkms/0.0.0-phase5.51/gate-d-execution-instance-v1.schema.json").is_file()
    for schema_name in ("gate-d-qualification-root-v1.schema.json", "gate-d-qualification-bootstrap-plan-v1.schema.json", "gate-d-target-plan-v1.schema.json", "gate-d-attempt-index-v1.schema.json", "gate-d-pre-root-bootstrap-envelope-v1.schema.json"):
        assert (target / "usr/share/rp1-gpclk-dkms/0.0.0-phase5.51" / schema_name).is_file()
