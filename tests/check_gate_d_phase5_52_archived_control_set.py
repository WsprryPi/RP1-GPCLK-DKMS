...
"""Validate Phase 5.52 controls with only exact frozen archived tool bytes."""
...
ARCHIVE_SHA256 = "0c67dee49a26bf5ab103d04bcf493bba8ae373a9f45b87e5704f52ede96bce01"
RELEASE = "0.0.0-phase5.52"

if len(sys.argv) != 2:
    raise SystemExit("usage: check_gate_d_phase5_52_archived_control_set.py ARCHIVE")
...
if archive.is_symlink() or not archive.is_file() or sha(archive) != ARCHIVE_SHA256:
    raise SystemExit("Phase 5.52 archive identity differs")

instance = json.loads((ROOT / "release/gate-d-execution-instance-phase5.52-v1.json").read_text())
plan = json.loads((ROOT / "release/gate-d-target-operation-plan-phase5.52-v1.json").read_text())
envelope = json.loads((ROOT / "release/gate-d-pre-root-bootstrap-envelope-phase5.52-v1.json").read_text())
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    with tarfile.open(archive, "r:gz") as source:
        ...
        for member in members:
            ...
            if (not pure.parts or pure.parts[0] != prefix or pure.is_absolute() or
                ...
                raise SystemExit("unsafe Phase 5.52 archive member")
    ...
    subprocess.run([
        ...
        str(ROOT / "release/gate-d-execution-instance-phase5.52-v1.json"),
    ], check=True)
    ...
    if instance["authorization"]["approved"] or instance["authorization"]["targetExecutionApproved"]:
        raise SystemExit("archived validator found authorization-bearing controls")
...
print("Phase 5.52 exact archived control-set validation: PASS")
