...
"""Validate the sealed Phase 5.32 Gate D control set entirely offline."""
...
COMMIT="4e62b3a0b584396a9528be07592d92e0796555f2"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.32-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.32-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.32-v1.json")
plan=load("release/gate-d-target-operation-plan-phase5.32-v1.json")
instance=load("release/gate-d-execution-instance-phase5.32-v1.json")
index=load("release/gate-d-attempts-phase5.32-v1/index.json")
assert gate_d_preroot.validate(envelope)=={"valid":True,"readOnly":True,"outputDisabled":True}
assert gate_d_root.validate(instance["qualificationRoot"],verify=False)==pathlib.Path("/home/pi/gate-d-qualification/phase5.32-4e62b3a0b584")
assert bootstrap["qualificationRoot"]==plan["qualificationRoot"]==index["qualificationRoot"]==instance["qualificationRoot"]
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.32"
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.32-v1.json")
assert route["candidate"]["sourceCommit"]==COMMIT
assert route["candidate"]["archiveSha256"]=="068a3c78011427f643c4880e9bb18d59c1d4bfdb812f82c76137ab64d2365bbe"
assert route["evidence"]["moduleSha256"]=="13fd2a026810cdb790f8b7ad04bb15fe93fed48dc1366b159ef3872ada84e715"
...
assert sum(x.startswith("release/gate-d-attempts-phase5.32-v1/gd-") for x in destinations)==38
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.32-v1"; documents=[]
...
with tempfile.TemporaryDirectory() as temporary:
 ...
 try:
  ...
  else: raise AssertionError("unauthorized Phase 5.32 instance became execution-ready")
  bad=copy.deepcopy(instance); bad["authorization"]["targetExecutionApproved"]=True; bad["executionReady"]=True
  assert hashlib.sha256((json.dumps(bad,indent=2,sort_keys=True)+"\n").encode()).hexdigest()!=sources["/home/pi/gate-d-inputs/phase5.32-4e62b3a0b584/control-set/release/gate-d-execution-instance-phase5.32-v1.json"]["sha256"]
...
print("Gate D Phase 5.32 offline control set: PASS")
