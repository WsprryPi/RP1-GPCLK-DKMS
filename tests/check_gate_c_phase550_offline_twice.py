...
"""Validate exact-freeze Phase 5.50 offline-checks-twice evidence."""
import hashlib
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
EVIDENCE = ROOT / "docs/evidence/gate-c-phase5.50-offline-checks-twice.json"
TRANSCRIPT = ROOT / "docs/evidence/gate-c-phase5.50-offline-checks-twice-transcript.txt"
value = json.loads(EVIDENCE.read_text())

assert value["release"] == "0.0.0-phase5.50"
assert value["sourceCommit"] == "c24160517b10900bf61243d4988f38247eeed58e"
...
assert digest == "2fdf92ca5a3c60539295d3bae4af0609f4a682b279baedb861e1da4c0c885cc3"
assert len(value["runs"]) == 2
assert all(run["exitStatus"] == 0 and run["passLines"] == 133 and
           run["skipLines"] == 3 and run["failLines"] == 0 and
           run["transcriptSha256"] == digest for run in value["runs"])
lines = TRANSCRIPT.read_text().splitlines()
assert len(lines) == 162
assert sum("PASS" in line for line in lines) == 133
...
assert [line.replace(": SKIP (", ": ").removesuffix(")")
        for line in lines if "SKIP" in line] == value["declaredSkips"]
assert value["representativeBuildValidator"] == {
    "commit":"739365a8bc3702da907aef00e6291b8658fe035c",
    "result":"passed",
    "execution":"independent-after-frozen-runs",
}
print("Phase 5.50 offline checks twice: PASS")
