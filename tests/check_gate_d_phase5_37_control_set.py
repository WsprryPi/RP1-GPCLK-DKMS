...
"""Validate the sealed Phase 5.37 Gate D control set entirely offline."""
...
COMMIT="71932324ec977d30ec0fadd48ef2673c49a6e173"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.37-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.37-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.37-v1.json")
plan=load("release/gate-d-target-operation-plan-phase5.37-v1.json")
instance=load("release/gate-d-execution-instance-phase5.37-v1.json")
index=load("release/gate-d-attempts-phase5.37-v1/index.json")
...
assert envelope["priorTerminalState"]=={
  "path":"/var/lib/rp1-gpclk-dkms/transaction.json",
  "sha256":"1ee3c83cbd88d8980ee0be5b1514939a8bc66953b74d966a5a6151f295e6a51e",
  "status":"recovered","recoveryRequired":False,"liveOutput":False,
  "ownerUid":0,"mode":"0600",
  "archivePath":"/var/lib/rp1-gpclk-dkms/history/phase5.36-transaction-recovered.json",
  "archiveMode":"0400",
}
assert "/var/lib/rp1-gpclk-dkms/history/phase5.34-transaction-recovered.json" not in envelope["cleanupPaths"]
assert gate_d_preroot.validate(envelope)=={"valid":True,"readOnly":True,"outputDisabled":True}
assert gate_d_root.validate(instance["qualificationRoot"],verify=False)==pathlib.Path("/home/pi/gate-d-qualification/phase5.37-71932324ec97")
assert bootstrap["qualificationRoot"]==plan["qualificationRoot"]==index["qualificationRoot"]==instance["qualificationRoot"]
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.37"
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.37-v1.json")
assert route["candidate"]["sourceCommit"]==COMMIT
assert route["candidate"]["archiveSha256"]=="299e8abe61f7c4ee81d9431539dcbae3614d33be2dafca0cb94a83996a4146ef"
assert route["evidence"]["moduleSha256"]=="d34c6369ff56c3aa281f023fe5b2f044c409a2117c2120e115bc536703a52add"
...
assert sum(x.startswith("release/gate-d-attempts-phase5.37-v1/gd-") for x in destinations)==38
...
identity=load("docs/evidence/gate-d-phase5.37-qualification-install-identity.json")
...
assert set(phase531).issubset(transitions) and len(transitions)==22
assert all(transitions[path]["predecessorSha256"]==phase531[path] for path in phase531)
omitted_phase536={
 "/usr/libexec/rp1-gpclk-dkms/gate-d-attempts":"dbb82601fddfcc820afec8e62677dc34a127a73aea0be0cc39743c8b031dd96c",
 "/usr/libexec/rp1-gpclk-dkms/gate-d-residue":"64e8c6e0bfb3bf99961230e9489359f093d2c1ccfcab630f28919f3bc2be4f4f",
 "/usr/libexec/rp1-gpclk-dkms/lifecycle-policy":"29075e6f15c04a5b1f7e2610b5dc23530228473f1285b7efdd999d319e53f6a7",
 "/usr/libexec/rp1-gpclk-dkms/rp1-gpclk-diagnostics":"0508c9ce09ebd7637fc73a7be0ca7cb7683b2d96443d209ae855378cfa0c6915",
}
assert set(transitions)-set(phase531)==set(omitted_phase536)
assert all(transitions[path]["predecessorSha256"]==value for path,value in omitted_phase536.items())
assert all(transitions[path]["successorSha256"]==installed_hashes[path] for path in transitions)
attempt_dir=ROOT/"release/gate-d-attempts-phase5.37-v1"; documents=[]
...
with tempfile.TemporaryDirectory() as temporary:
 ...
 try:
  ...
  else: raise AssertionError("unauthorized instance passed readiness-required validation")
  ...
  assert hashlib.sha256((json.dumps(bad,indent=2,sort_keys=True)+"\n").encode()).hexdigest()!=sources["/home/pi/gate-d-inputs/phase5.37-71932324ec97/control-set/release/gate-d-execution-instance-phase5.37-v1.json"]["sha256"]
...
print("Gate D Phase 5.37 offline control set: PASS")
