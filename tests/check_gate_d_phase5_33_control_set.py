...
"""Validate the sealed Phase 5.33 Gate D control set entirely offline."""
...
COMMIT="4208941af537f21e3a20160d2d3d7fabe50f7cd3"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.33-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.33-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.33-v1.json")
...
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.33"
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.33-v1.json")
...
assert sum(x.startswith("release/gate-d-attempts-phase5.33-v1/gd-") for x in destinations)==38
...
identity=load("docs/evidence/gate-d-phase5.33-qualification-install-identity.json")
...
assert all(transitions[path]["predecessorSha256"]==json.loads((ROOT/"release/gate-d-qualification-bootstrap-plan-phase5.32-v1.json").read_text())["retainedTools"][i]["sha256"] for i,path in enumerate(x["path"] for x in json.loads((ROOT/"release/gate-d-qualification-bootstrap-plan-phase5.32-v1.json").read_text())["retainedTools"]))
assert all(transitions[path]["successorSha256"]==installed_hashes[path] for path in transitions)
attempt_dir=ROOT/"release/gate-d-attempts-phase5.33-v1"; documents=[]
...
print("Gate D Phase 5.33 offline control set: PASS")
