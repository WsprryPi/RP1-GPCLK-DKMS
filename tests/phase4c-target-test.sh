...
work=$(mktemp -d /tmp/rp1-gpclk-phase4c-live.XXXXXX)
...
{
	set +e
	pkill -f "$work/phase4c_sdr_capture" 2>/dev/null
...
cc -std=c11 -Wall -Wextra -Werror -I"$source_dir/include/uapi" \
	"$source_dir/tests/phase4c_live_client.c" -lm -o "$work/phase4c_live_client"
g++ -std=c++17 -Wall -Wextra -Werror "$source_dir/tests/phase4c_sdr_capture.cpp" \
	$(pkg-config --cflags --libs SoapySDR) -o "$work/phase4c_sdr_capture"

openssl req -new -x509 -newkey rsa:2048 -nodes -days 1 \
	-subj /CN=RP1-GPCLK-Phase4C-GPIO20/ -keyout "$work/signing-key.pem" \
...
# GPIO4 is administratively probed but must not enroll or expose an endpoint.
...
sleep 1
"$work/phase4c_live_client" query | tee "$evidence_dir/query.txt"

run_capture()
{
	local name=$1 mode=$2 seconds=$3 before pid
	before=$(dmesg | wc -l)
	"$work/phase4c_sdr_capture" "$evidence_dir/$name.cf32" "$seconds" \
	...
	"$work/phase4c_live_client" "$mode" >"$evidence_dir/$name-client.txt"
	wait "$pid"
	dmesg | tail -n "+$((before + 1))" >"$evidence_dir/$name-dmesg.txt"
	grep 'phase4c generation=' "$evidence_dir/$name-dmesg.txt" \
...
python3 "$source_dir/tests/phase4c_analyze.py" "$evidence_dir" \
...
[[ $(grep -c 'phase4c generation=' "$evidence_dir/dmesg-live-delta.txt") -eq 13 ]]
! grep -Eq 'BUG:|WARNING:|Oops:|Call trace:|phase4c cleanup:|cleanup=-[1-9]' \
	"$evidence_dir/dmesg-live-delta.txt"
...
echo PHASE4C_GPIO20_TARGET_RESULT=PASS
