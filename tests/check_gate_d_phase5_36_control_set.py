...
"""Validate the sealed Phase 5.36 Gate D control set entirely offline."""
...
COMMIT="20f7a21ad8601f2e2fd4dec4640ea919acc22ce0"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.36-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.36-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.36-v1.json")
plan=load("release/gate-d-target-operation-plan-phase5.36-v1.json")
instance=load("release/gate-d-execution-instance-phase5.36-v1.json")
index=load("release/gate-d-attempts-phase5.36-v1/index.json")
...
assert envelope["priorTerminalState"]=={
  "path":"/var/lib/rp1-gpclk-dkms/transaction.json",
  "sha256":"48946aff65cc88a765510e86891fc9b67de71fa9b651a8ba0198893a053d2afa",
  "status":"recovered","recoveryRequired":False,"liveOutput":False,
  "ownerUid":0,"mode":"0600",
  "archivePath":"/var/lib/rp1-gpclk-dkms/history/phase5.34-transaction-recovered.json",
  "archiveMode":"0400",
}
assert gate_d_preroot.validate(envelope)=={"valid":True,"readOnly":True,"outputDisabled":True}
assert gate_d_root.validate(instance["qualificationRoot"],verify=False)==pathlib.Path("/home/pi/gate-d-qualification/phase5.36-20f7a21ad860")
assert bootstrap["qualificationRoot"]==plan["qualificationRoot"]==index["qualificationRoot"]==instance["qualificationRoot"]
assert route["candidate"]["release"]==instance["candidate"]["release"]==plan["artifacts"]["successor"]["version"]=="0.0.0-phase5.36"
assert route["candidate"]["representativeBuildManifestSha256"]==sha(ROOT/"release/gate-c-representative-build-manifest-phase5.36-v1.json")
assert route["candidate"]["sourceCommit"]==COMMIT
assert route["candidate"]["archiveSha256"]=="a5d9fa6d83a4ea7405ede432be0bfcea201d850d21b3860fc40931f7e2fef271"
assert route["evidence"]["moduleSha256"]=="c11f89a63c4e2fbe09f6f0a401df348cbe5f4d713747f8c105a7731dc1007909"
...
assert sum(x.startswith("release/gate-d-attempts-phase5.36-v1/gd-") for x in destinations)==38
...
identity=load("docs/evidence/gate-d-phase5.36-qualification-install-identity.json")
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.36-v1"; documents=[]
...
with tempfile.TemporaryDirectory() as temporary:
 ...
 try:
  ...
  else: raise AssertionError("unauthorized Phase 5.36 instance accepted as execution-ready")
  ...
  assert hashlib.sha256((json.dumps(bad,indent=2,sort_keys=True)+"\n").encode()).hexdigest()!=sources["/home/pi/gate-d-inputs/phase5.36-20f7a21ad860/control-set/release/gate-d-execution-instance-phase5.36-v1.json"]["sha256"]
...
print("Gate D Phase 5.36 offline control set: PASS")
