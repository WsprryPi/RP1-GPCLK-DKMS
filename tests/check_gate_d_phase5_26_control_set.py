...
"""Validate the sealed Phase 5.26 Gate D control set entirely offline."""
...
COMMIT="9f009240eecd55940d53d6f13cb9567aa76cd4ce"
def load(relative):
...
def frozen_payload(relative,expected):
 ...
 return payload
envelope=load("release/gate-d-pre-root-bootstrap-envelope-phase5.26-v1.json")
bootstrap=load("release/gate-d-qualification-bootstrap-plan-phase5.26-v1.json")
route=load("release/gate-d-route-compatibility-decision-phase5.26-v1.json")
...
assert sum(x.startswith("release/gate-d-attempts-phase5.26-v1/gd-") for x in destinations)==38
...
attempt_dir=ROOT/"release/gate-d-attempts-phase5.26-v1"; documents=[]
...
print("Gate D Phase 5.26 offline control set: PASS")
