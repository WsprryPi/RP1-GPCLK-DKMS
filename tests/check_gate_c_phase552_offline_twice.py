...
"""Validate exact-freeze Phase 5.52 offline-checks-twice evidence."""
import hashlib
import json
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
EVIDENCE = ROOT / "docs/evidence/gate-c-phase5.52-offline-checks-twice.json"
TRANSCRIPT = ROOT / "docs/evidence/gate-c-phase5.52-offline-checks-twice-transcript.txt"
value = json.loads(EVIDENCE.read_text())
assert value["release"] == "0.0.0-phase5.52"
assert value["sourceCommit"] == "f710554c4697d75210cbd33c9eea13474d60557a"
...
assert value["archivedReleaseInputs"] == [
    ...
    {"release":"0.0.0-phase5.51","sha256":"253bd54054eb0b673f9e61c58a46b6b7ca2cf78d756fe4e80656f4ac1233f549"},
]
digest = hashlib.sha256(TRANSCRIPT.read_bytes()).hexdigest()
assert digest == "69c4e07ce578b002c32f186bffe7e0dc4bb635960f1a047db19bf6a9727190ca"
assert value["transcriptsByteIdentical"] is True and value["result"] == "passed"
assert len(value["runs"]) == 2
assert all(run["exitStatus"] == 0 and run["passLines"] == 157 and
           run["skipLines"] == 3 and run["failLines"] == 0 and
           run["transcriptSha256"] == digest for run in value["runs"])
lines = TRANSCRIPT.read_text().splitlines()
assert len(lines) == 200 and sum("PASS" in line for line in lines) == 157
...
for release in value["archivedValidatorsPassed"]:
    ...
    label = "pre-root envelope" if phase not in {"5.50", "5.51"} else "control-set"
    assert f"Phase {phase} exact archived {label} validation: PASS" in lines
assert [line.replace(": SKIP (", ": ").removesuffix(")") for line in lines
        if "SKIP" in line] == value["declaredSkips"]
assert value["representativeBuildValidator"] == {
    "commit":"376e930b28eb7cb9dda78acca30afb8e4332793d",
    "result":"passed", "execution":"independent-after-frozen-runs"}
assert value["rejectedRuns"] == {"count":0, "reason":"none"}
print("Phase 5.52 offline checks twice: PASS")
