...
"""Independently validate the complete Phase 5.45 Gate D control set."""
...
snapshot_path = ROOT / "docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.45-v1.json"
snapshot = load("docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.45-v1.json")
build = load("release/gate-c-representative-build-manifest-phase5.45-v1.json")
route = load("release/gate-d-route-compatibility-decision-phase5.45-v1.json")
plan = load("release/gate-d-target-operation-plan-phase5.45-v1.json")
bootstrap = load("release/gate-d-qualification-bootstrap-plan-phase5.45-v1.json")
instance = load("release/gate-d-execution-instance-phase5.45-v1.json")
envelope = load("release/gate-d-pre-root-bootstrap-envelope-phase5.45-v1.json")
identity = load("docs/evidence/gate-d-phase5.45-qualification-install-identity.json")
inventory = load("docs/evidence/gate-d-phase5.45-predecessor-package-inventory.json")

assert sha(snapshot_path) == "66208586a112792e91185a7ce67d5952427dc218fb8a392ac1bfb221ed51e4c8"
...
for record in index["attempts"]:
    ...
    namespace = "/gate-d/runs/phase5.45-4b50db7868b7/"
...
subprocess.run([sys.executable, str(ROOT / "scripts/gate_d_live_snapshot_validate.py"),
                ...
                str(ROOT / "release/gate-d-pre-root-bootstrap-envelope-phase5.45-v1.json"),
                "--inventory", str(ROOT / "docs/evidence/gate-d-phase5.45-predecessor-package-inventory.json"),
                "--route", str(ROOT / "release/gate-d-route-compatibility-decision-phase5.45-v1.json"),
                "--build", str(ROOT / "release/gate-c-representative-build-manifest-phase5.45-v1.json")],
               check=True)
subprocess.run([sys.executable, str(ROOT / "scripts/generate_phase5_45_control_set.py"),
                "--check"], check=True)
print("Gate D Phase 5.45 offline control set: PASS")
