...
"""Validate the Phase 5.48 envelope with exact frozen archive tools."""
...
def main() -> None:
    ...
    manifest = json.loads((ROOT / "release/gate-c-representative-build-manifest-phase5.48-v1.json").read_text())
    envelope = json.loads((ROOT / "release/gate-d-pre-root-bootstrap-envelope-phase5.48-v1.json").read_text())
    ...
    index = json.loads((ROOT / "release/gate-d-attempts-phase5.48-v1/index.json").read_text())
    if sha(archive) != manifest["candidate"]["archiveSha256"]:
        raise SystemExit("Phase 5.48 release archive identity differs")
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        with tarfile.open(archive, "r:gz") as unit:
            for name in members.values():
                ...
                if not member.isfile() or member.issym() or member.islnk():
                    raise SystemExit("Phase 5.48 archived tool is not a regular file")
        ...
        spec = importlib.util.spec_from_file_location("phase5_48_archived_preroot", preroot)
        if spec is None or spec.loader is None:
            raise SystemExit("cannot load archived Phase 5.48 pre-root module")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        assert module.validate(envelope) == {"valid": True, "readOnly": True,
                                             "outputDisabled": True}
    print("Phase 5.48 exact archived pre-root envelope validation: PASS")
