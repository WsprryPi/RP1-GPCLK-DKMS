...
"""Deterministically generate the snapshot-bound Phase 5.46 Gate D controls."""
...
import generate_phase5_45_control_set as predecessor_generator

ROOT = pathlib.Path(__file__).resolve().parents[1]
OLD_RELEASE = "0.0.0-phase5.45"
RELEASE = "0.0.0-phase5.46"
OLD_COMMIT = "4b50db7868b7fe5ca9d830f51cd404c250192188"
COMMIT = "b43e2744b212f5bc53ad40584254f52310af4684"
OLD_SUFFIX = "phase5.45-4b50db7868b7"
SUFFIX = "phase5.46-b43e2744b212"
NAMESPACE = SUFFIX
SNAPSHOT = ROOT / "docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.46-v1.json"
BUILD = ROOT / "release/gate-c-representative-build-manifest-phase5.46-v1.json"
...
def generate(output_root: pathlib.Path) -> list[pathlib.Path]:
    ...
    if file_sha(SNAPSHOT) != "bc4c307350d6e74c9cbb85ef890fbaf0e8ad969ecdeb661a98703b70bd4a1859":
        raise ValueError("Phase 5.46 canonical snapshot differs")
    build = json.loads(BUILD.read_text())
    inventory = json.loads(INVENTORY.read_text())
    artifacts = {item["name"]: item for item in inventory["artifacts"]}
    old_inventory = json.loads((ROOT / "docs/evidence/gate-c-phase5.45-release-input-inventory.json").read_text())
    old_artifacts = {item["name"]: item for item in old_inventory["artifacts"]}
    replacements = {
        OLD_SUFFIX: SUFFIX, OLD_RELEASE: RELEASE, "phase5.45": "phase5.46",
        OLD_COMMIT: COMMIT,
        "21d05675e9d12ddb4c051868578c410737b733786357cee20eb2b0ce03f63356": build["candidate"]["archiveSha256"],
        "977c6997fd87dfb68c61ab4b82db904e86310083741d3a41c0405a417aa36d95": build["result"]["moduleSha256"],
        file_sha(ROOT / "release/gate-c-representative-build-manifest-phase5.45-v1.json"): file_sha(BUILD),
        "66208586a112792e91185a7ce67d5952427dc218fb8a392ac1bfb221ed51e4c8": file_sha(SNAPSHOT),
        **source_replacements(),
    }
    for old_name, old_item in old_artifacts.items():
        new_name = old_name.replace("phase5.45", "phase5.46")
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for name in CONTROL_NAMES:
            old_path = old_root / f"release/{name}-phase5.45-v1.json"
    ...
    for entry in route["routes"]:
        entry["reason"] = ("Exact Phase 5.46 representative build and canonical inactive "
    ...
    identity_rel = "docs/evidence/gate-d-phase5.46-qualification-install-identity.json"
    identity_path = output_root / identity_rel
    identity_path.parent.mkdir(parents=True, exist_ok=True)
    identity_path.write_bytes(pretty(identity))

    inventory_rel = "docs/evidence/gate-d-phase5.46-predecessor-package-inventory.json"
    ...
    route_rel = "release/gate-d-route-compatibility-decision-phase5.46-v1.json"
    route_path = output_root / route_rel
    route_path.parent.mkdir(parents=True, exist_ok=True)
    route_path.write_bytes(pretty(route))

    bootstrap_rel = "release/gate-d-qualification-bootstrap-plan-phase5.46-v1.json"
    ...
    plan_path = output_root / "release/gate-d-target-operation-plan-phase5.46-v1.json"
    ...
    instance["authorization"].update(
        approved=True, targetExecutionApproved=False,
        approvalScope=("Operator-approved offline construction and validation only. Target staging and "
                       "lifecycle execution require later explicit authorization bound to "
                       "the committed Phase 5.46 control-set bytes."))
    ...
    instance["executionPolicy"].update(
        ...
        targetPlan="release/gate-d-target-operation-plan-phase5.46-v1.json",
        ...
        attemptIndex="release/gate-d-attempts-phase5.46-v1/index.json")
    for row in instance["rows"]:
        row["evidenceDirectory"] = f"gate-d/runs/{NAMESPACE}/{row['id']}"

    attempts = gate_d_attempts.generate(instance, plan)
    attempt_dir = output_root / "release/gate-d-attempts-phase5.46-v1"
    ...
    instance_path = output_root / "release/gate-d-execution-instance-phase5.46-v1.json"
    ...
    envelope["priorTerminalState"] = {
        ...
        "mode": ledger["mode"],
        "archivePath": "/var/lib/rp1-gpclk-dkms/recovery/phase5.46-phase5.45-transaction-complete.json",
        "archiveMode": "0400"}
    roles = {"rp1-gpclk-dkms-0.0.0-phase5.46.tar.gz": "archive",
    ...
    envelope_path = output_root / "release/gate-d-pre-root-bootstrap-envelope-phase5.46-v1.json"
...
def main() -> None:
    ...
    if args.check:
        ...
        print("Phase 5.46 control-set deterministic generation: PASS")
    else:
        print(f"generated {len(generate(ROOT))} Phase 5.46 control documents")
