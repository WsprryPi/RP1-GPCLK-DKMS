...
"""Deterministically generate the snapshot-bound Phase 5.52 Gate D controls."""
...
import generate_phase5_51_control_set as predecessor_generator

ROOT = pathlib.Path(__file__).resolve().parents[1]
OLD_RELEASE = "0.0.0-phase5.51"
RELEASE = "0.0.0-phase5.52"
OLD_COMMIT = "cc87e0cdec7195eb69de2a6606f388e23ee0799c"
COMMIT = "f710554c4697d75210cbd33c9eea13474d60557a"
OLD_SUFFIX = "phase5.51-cc87e0cdec71"
SUFFIX = "phase5.52-f710554c4697"
NAMESPACE = SUFFIX
SNAPSHOT = ROOT / "docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.52-v1.json"
BUILD = ROOT / "release/gate-c-representative-build-manifest-phase5.52-v1.json"
...
def generate(output_root: pathlib.Path) -> list[pathlib.Path]:
    ...
    if file_sha(SNAPSHOT) != "449201a0a51ca8b278b7ae077410e515aa9c176eac42f4ba86bd62ef4c36451f":
        raise ValueError("Phase 5.52 canonical snapshot differs")
    build = json.loads(BUILD.read_text())
    inventory = json.loads(INVENTORY.read_text())
    artifacts = {item["name"]: item for item in inventory["artifacts"]}
    old_inventory = json.loads((ROOT / "docs/evidence/gate-c-phase5.51-release-input-inventory.json").read_text())
    old_artifacts = {item["name"]: item for item in old_inventory["artifacts"]}
    replacements = {
        OLD_SUFFIX: SUFFIX, OLD_RELEASE: RELEASE, "phase5.51": "phase5.52",
        OLD_COMMIT: COMMIT,
        "253bd54054eb0b673f9e61c58a46b6b7ca2cf78d756fe4e80656f4ac1233f549": build["candidate"]["archiveSha256"],
        "5fcfcc41e44a3685b7051b7ea8fbcce67f0fa79fefb29b8e203a231d7295d192": build["result"]["moduleSha256"],
        file_sha(ROOT / "release/gate-c-representative-build-manifest-phase5.51-v1.json"): file_sha(BUILD),
        "badb3633cdf6bacce6fa3292174d3cea993a1ed4f0278f75c597bd204ee63e9a": file_sha(SNAPSHOT),
        **source_replacements(),
    }
    for old_name, old_item in old_artifacts.items():
        new_name = old_name.replace("phase5.51", "phase5.52")
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for name in CONTROL_NAMES:
            old_path = old_root / f"release/{name}-phase5.51-v1.json"
    ...
    for entry in route["routes"]:
        entry["reason"] = ("Exact Phase 5.52 representative build and canonical inactive "
    ...
    identity_rel = "docs/evidence/gate-d-phase5.52-qualification-install-identity.json"
    identity_path = output_root / identity_rel
    identity_path.parent.mkdir(parents=True, exist_ok=True)
    identity_path.write_bytes(pretty(identity))

    inventory_rel = "docs/evidence/gate-d-phase5.52-predecessor-package-inventory.json"
    ...
    route_rel = "release/gate-d-route-compatibility-decision-phase5.52-v1.json"
    route_path = output_root / route_rel
    route_path.parent.mkdir(parents=True, exist_ok=True)
    route_path.write_bytes(pretty(route))

    bootstrap_rel = "release/gate-d-qualification-bootstrap-plan-phase5.52-v1.json"
    ...
    plan_path = output_root / "release/gate-d-target-operation-plan-phase5.52-v1.json"
    ...
    instance["executionPolicy"].update(
        ...
        targetPlan="release/gate-d-target-operation-plan-phase5.52-v1.json",
        ...
        attemptIndex="release/gate-d-attempts-phase5.52-v1/index.json")
    for row in instance["rows"]:
        row["evidenceDirectory"] = f"gate-d/runs/{NAMESPACE}/{row['id']}"

    attempts = gate_d_attempts.generate(instance, plan, schema_version=2)
    attempt_dir = output_root / "release/gate-d-attempts-phase5.52-v1"
    ...
    instance_path = output_root / "release/gate-d-execution-instance-phase5.52-v1.json"
    ...
    envelope["priorTerminalState"] = {
        ...
        "mode": ledger["mode"],
        "archivePath": "/var/lib/rp1-gpclk-dkms/recovery/phase5.52-phase5.51-transaction-complete.json",
        "archiveMode": "0400"}
    roles = {"rp1-gpclk-dkms-0.0.0-phase5.52.tar.gz": "archive",
    ...
    envelope_path = output_root / "release/gate-d-pre-root-bootstrap-envelope-phase5.52-v1.json"
...
def main() -> None:
    ...
    if args.check:
        ...
        print("Phase 5.52 control-set deterministic generation: PASS")
    else:
        print(f"generated {len(generate(ROOT))} Phase 5.52 control documents")
