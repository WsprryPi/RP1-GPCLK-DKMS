...
"""Deterministically generate the Phase 5.42 Gate D control set."""
...
TEMPLATE_COMMIT = "05e9b54b049886d51fae8b01c44d677ce86cc276"
SOURCE_COMMIT = "5dc05b6e10cdb50c4f937b484fc92cf4469e54ab"
OLD_SOURCE_COMMIT = "640877c1f29297e2f6ea855742605550781256e9"
ARCHIVE_SHA256 = "a6baa472e907135b9066c6bbb2bceee6ec849025d7d7b157d93a45297f6c5f54"
MODULE_SHA256 = "44ec33c8dd6ae06eeb5476e43ffbc9f0359172c184b25a04d962c83fa4e4ed61"
OLD_ARCHIVE_SHA256 = "b49cd75baefdb245d6d00e60cd171ba6fa4da4c00e63b07e925cdd52f0b0934f"
OLD_MODULE_SHA256 = "2ad72b226f93c1d9ac859dafc2b01b87a86a9bba6f6ffc28fe381e4ed6ab4070"
OLD_ROOT_SUFFIX = "phase5.41-640877c1f292"
NEW_ROOT_SUFFIX = "phase5.42-5dc05b6e10cd"
CONTROL_NAMES = (
...
RELEASE_NAMES = {
    ...
    "rp1-gpclk-dkms-0.0.0-phase5.42.tar.gz",
...
def source_hash_replacements() -> dict[str, str]:
    ...
    for path in sorted(paths):
        old_path = path
        new_path = path.replace("phase5.41", "phase5.42")
        try:
...
def release_artifacts() -> dict[str, dict]:
    path = ROOT / "docs/evidence/gate-c-phase5.42-release-input-inventory.json"
    value = json.loads(path.read_text())
    if (value.get("host") != "wspr5" or value.get("release") != "0.0.0-phase5.42" or
            value.get("sourceCommit") != SOURCE_COMMIT or
            value.get("directory") != "/home/pi/gate-c-evidence/phase5.42-5dc05b6"):
...
def generate(output_root: pathlib.Path) -> list[pathlib.Path]:
    ...
    replacements = {
        ...
        "phase5.41": "phase5.42",
        "0.0.0-phase5.41": "0.0.0-phase5.42",
    ...
    stale_release_hashes = {
        "0d306c02c28dc8d47c0096e09e2226f99997e6fbee88d035441e089e0cdcf927": "PROVENANCE.json",
        "6e0a265577fd1b2d1a75672791ade733026089b29e746b034edac7d933a08c87": "SHA256SUMS",
        "78877e54204d09e0659f59a41af76070884119a847d551a844e74a818ba11986": "release-metadata.json",
        "06f28ff52df58e465c479731e86dad04329b2dd444277b0f906612a1105d1b94": "rp1-gpclk-compatibility-manifest.json",
    }
    ...
    replacements[sha(git_bytes(TEMPLATE_COMMIT,
        "release/gate-c-representative-build-manifest-phase5.41-v1.json"))] = sha(
        (ROOT / "release/gate-c-representative-build-manifest-phase5.42-v1.json").read_bytes())
    generated: list[tuple[str, str, pathlib.Path]] = []
    for name in CONTROL_NAMES:
        old = f"release/{name}-phase5.41-v1.json"
        new = f"release/{name}-phase5.42-v1.json"
    ...
    index_old = "release/gate-d-attempts-phase5.41-v1/index.json"
    ...
    for filename in [*attempt_names, "index.json"]:
        old = f"release/gate-d-attempts-phase5.41-v1/{filename}"
        new = f"release/gate-d-attempts-phase5.42-v1/{filename}"
        destination = output_root / new
        destination.parent.mkdir(parents=True, exist_ok=True)
        destination.write_bytes(transform(git_bytes(TEMPLATE_COMMIT, old), replacements))
        generated.append((old, new, destination))
    ...
    inventory_old = "docs/evidence/gate-d-phase5.41-predecessor-package-inventory.json"
    inventory_new = "docs/evidence/gate-d-phase5.42-predecessor-package-inventory.json"
    ...
    inventory_path.write_bytes(pretty({
        ...
        "capturedFor": "0.0.0-phase5.42", "paths": predecessor,
    }))
    ...
    identity = {
        ...
        "release": "0.0.0-phase5.42", "sourceCommit": SOURCE_COMMIT,
    ...
    identity_old = "docs/evidence/gate-d-phase5.41-qualification-install-identity.json"
    identity_new = "docs/evidence/gate-d-phase5.42-qualification-install-identity.json"
    ...
    marker = {
        ...
        "candidateRelease": "0.0.0-phase5.42", "sourceCommit": SOURCE_COMMIT,
    }
        ...
        if isinstance(value, dict):
            ...
            if value.get("kind") == "gate-d-pre-root-bootstrap-envelope":
                ...
                value["priorTerminalState"] = {
                    ...
                    "archivePath": "/var/lib/rp1-gpclk-dkms/recovery/phase5.39-transaction-complete.json",
                    "archiveMode": "0400",
                }
                changed = True
...
def main() -> None:
    ...
    if args.check:
        ...
        print("Phase 5.42 control-set deterministic generation: PASS")
    else:
        generated = generate(ROOT)
        print(f"generated {len(generated)} Phase 5.42 control documents")
