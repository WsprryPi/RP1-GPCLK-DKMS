...
"""Deterministically generate the snapshot-bound Phase 5.47 Gate D controls."""
...
import generate_phase5_46_control_set as predecessor_generator

ROOT = pathlib.Path(__file__).resolve().parents[1]
OLD_RELEASE = "0.0.0-phase5.46"
RELEASE = "0.0.0-phase5.47"
OLD_COMMIT = "b43e2744b212f5bc53ad40584254f52310af4684"
COMMIT = "c5320ac5419a04d17345370204524f219b7ff403"
OLD_SUFFIX = "phase5.46-b43e2744b212"
SUFFIX = "phase5.47-c5320ac5419a"
NAMESPACE = SUFFIX
SNAPSHOT = ROOT / "docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.47-v1.json"
BUILD = ROOT / "release/gate-c-representative-build-manifest-phase5.47-v1.json"
...
def generate(output_root: pathlib.Path) -> list[pathlib.Path]:
    ...
    if file_sha(SNAPSHOT) != "7f018fb331c769c44eb1691fdabc4a8a6ff22e0a3debc9e1ed69404d829332b0":
        raise ValueError("Phase 5.47 canonical snapshot differs")
    build = json.loads(BUILD.read_text())
    inventory = json.loads(INVENTORY.read_text())
    artifacts = {item["name"]: item for item in inventory["artifacts"]}
    old_inventory = json.loads((ROOT / "docs/evidence/gate-c-phase5.46-release-input-inventory.json").read_text())
    old_artifacts = {item["name"]: item for item in old_inventory["artifacts"]}
    replacements = {
        OLD_SUFFIX: SUFFIX, OLD_RELEASE: RELEASE, "phase5.46": "phase5.47",
        OLD_COMMIT: COMMIT,
        "0e0debdd96560602bd61457afc59782cfad2a4fb1b6f9b54e0d2505453e6c8f2": build["candidate"]["archiveSha256"],
        "c1203555194b6d7983ca4bde978709f09588878022ea58df8fc90adda23ce6e7": build["result"]["moduleSha256"],
        file_sha(ROOT / "release/gate-c-representative-build-manifest-phase5.46-v1.json"): file_sha(BUILD),
        "bc4c307350d6e74c9cbb85ef890fbaf0e8ad969ecdeb661a98703b70bd4a1859": file_sha(SNAPSHOT),
        **source_replacements(),
    }
    for old_name, old_item in old_artifacts.items():
        new_name = old_name.replace("phase5.46", "phase5.47")
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for name in CONTROL_NAMES:
            old_path = old_root / f"release/{name}-phase5.46-v1.json"
    ...
    for entry in route["routes"]:
        entry["reason"] = ("Exact Phase 5.47 representative build and canonical inactive "
    ...
    identity_rel = "docs/evidence/gate-d-phase5.47-qualification-install-identity.json"
    identity_path = output_root / identity_rel
    identity_path.parent.mkdir(parents=True, exist_ok=True)
    identity_path.write_bytes(pretty(identity))

    inventory_rel = "docs/evidence/gate-d-phase5.47-predecessor-package-inventory.json"
    ...
    route_rel = "release/gate-d-route-compatibility-decision-phase5.47-v1.json"
    route_path = output_root / route_rel
    route_path.parent.mkdir(parents=True, exist_ok=True)
    route_path.write_bytes(pretty(route))

    bootstrap_rel = "release/gate-d-qualification-bootstrap-plan-phase5.47-v1.json"
    ...
    plan_path = output_root / "release/gate-d-target-operation-plan-phase5.47-v1.json"
    ...
    instance["authorization"].update(
        ...
        approvalScope=("Operator-authorized exact Phase 5.47 output-disabled Gate D scope "
                       "committed at f307eac68aeee19abd096a7e3ea975c58e9ad457; limited to "
    ...
    instance["executionPolicy"].update(
        ...
        targetPlan="release/gate-d-target-operation-plan-phase5.47-v1.json",
        ...
        attemptIndex="release/gate-d-attempts-phase5.47-v1/index.json")
    for row in instance["rows"]:
        row["evidenceDirectory"] = f"gate-d/runs/{NAMESPACE}/{row['id']}"

    attempts = gate_d_attempts.generate(instance, plan)
    attempt_dir = output_root / "release/gate-d-attempts-phase5.47-v1"
    ...
    instance_path = output_root / "release/gate-d-execution-instance-phase5.47-v1.json"
    ...
    envelope["priorTerminalState"] = {
        ...
        "mode": ledger["mode"],
        "archivePath": "/var/lib/rp1-gpclk-dkms/recovery/phase5.47-phase5.45-transaction-complete.json",
        "archiveMode": "0400"}
    roles = {"rp1-gpclk-dkms-0.0.0-phase5.47.tar.gz": "archive",
    ...
    envelope_path = output_root / "release/gate-d-pre-root-bootstrap-envelope-phase5.47-v1.json"
...
def main() -> None:
    ...
    if args.check:
        ...
        print("Phase 5.47 control-set deterministic generation: PASS")
    else:
        print(f"generated {len(generate(ROOT))} Phase 5.47 control documents")
