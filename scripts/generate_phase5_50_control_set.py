...
"""Deterministically generate the snapshot-bound Phase 5.50 Gate D controls."""
...
import generate_phase5_48_control_set as predecessor_generator

ROOT = pathlib.Path(__file__).resolve().parents[1]
OLD_RELEASE = "0.0.0-phase5.48"
RELEASE = "0.0.0-phase5.50"
OLD_COMMIT = "ef96f246b66b25bb70536341b60a5f1e64708c65"
COMMIT = "c24160517b10900bf61243d4988f38247eeed58e"
OLD_SUFFIX = "phase5.48-ef96f246b66b"
SUFFIX = "phase5.50-c24160517b10"
NAMESPACE = SUFFIX
SNAPSHOT = ROOT / "docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.50-v1.json"
...
def generate(output_root: pathlib.Path) -> list[pathlib.Path]:
    ...
    if file_sha(SNAPSHOT) != "3db85e0ada2427c997cdca878d0f2d53fc0c61d5aab68761e65a2a6cf349a6d5":
        raise ValueError("Phase 5.50 canonical snapshot differs")
    build = json.loads(BUILD.read_text())
    inventory = json.loads(INVENTORY.read_text())
    artifacts = {item["name"]: item for item in inventory["artifacts"]}
    old_inventory = json.loads((ROOT / "docs/evidence/gate-c-phase5.48-release-input-inventory.json").read_text())
    old_artifacts = {item["name"]: item for item in old_inventory["artifacts"]}
    replacements = {
        OLD_SUFFIX: SUFFIX, OLD_RELEASE: RELEASE, "phase5.48": "phase5.50",
        OLD_COMMIT: COMMIT,
        "18418395eac577d8718c1e74f6601e005160d2768ea7634a35d00e4ddead9120": build["candidate"]["archiveSha256"],
        "3ee865f9293b69f45f5c17a9217896a2d68c2addd7c494088b430aecb3faf615": build["result"]["moduleSha256"],
        file_sha(ROOT / "release/gate-c-representative-build-manifest-phase5.48-v1.json"): file_sha(BUILD),
        "9377d109235305f29e85134026cf9247e5d462b0bd2c8e05d9c0463162156e33": file_sha(SNAPSHOT),
        **source_replacements(),
    }
    for old_name, old_item in old_artifacts.items():
        new_name = old_name.replace("phase5.48", "phase5.50")
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for name in CONTROL_NAMES:
            old_path = old_root / f"release/{name}-phase5.48-v1.json"
    ...
    for entry in route["routes"]:
        entry["reason"] = ("Exact Phase 5.50 representative build and canonical inactive "
    ...
    identity_rel = "docs/evidence/gate-d-phase5.50-qualification-install-identity.json"
    identity_path = output_root / identity_rel
    identity_path.parent.mkdir(parents=True, exist_ok=True)
    identity_path.write_bytes(pretty(identity))

    inventory_rel = "docs/evidence/gate-d-phase5.50-predecessor-package-inventory.json"
    ...
    route_rel = "release/gate-d-route-compatibility-decision-phase5.50-v1.json"
    route_path = output_root / route_rel
    route_path.parent.mkdir(parents=True, exist_ok=True)
    route_path.write_bytes(pretty(route))

    bootstrap_rel = "release/gate-d-qualification-bootstrap-plan-phase5.50-v1.json"
    ...
    plan_path = output_root / "release/gate-d-target-operation-plan-phase5.50-v1.json"
    ...
    instance["authorization"].update(
        ...
        approvalScope=("Operator-authorized exact Phase 5.50 output-disabled Gate D scope "
                       "committed at 579910954a9f495e877cdc2c74b752f9e7005937; limited "
                       ...
                       "predecessor and successor inventories, authenticated schema-5 "
    ...
    instance["executionPolicy"].update(
        ...
        targetPlan="release/gate-d-target-operation-plan-phase5.50-v1.json",
        ...
        attemptIndex="release/gate-d-attempts-phase5.50-v1/index.json")
    for row in instance["rows"]:
        row["evidenceDirectory"] = f"gate-d/runs/{NAMESPACE}/{row['id']}"

    attempts = gate_d_attempts.generate(instance, plan, schema_version=2)
    attempt_dir = output_root / "release/gate-d-attempts-phase5.50-v1"
    ...
    instance_path = output_root / "release/gate-d-execution-instance-phase5.50-v1.json"
    ...
    envelope["priorTerminalState"] = {
        ...
        "mode": ledger["mode"],
        "archivePath": "/var/lib/rp1-gpclk-dkms/recovery/phase5.50-phase5.48-transaction-complete.json",
        "archiveMode": "0400"}
    roles = {"rp1-gpclk-dkms-0.0.0-phase5.50.tar.gz": "archive",
    ...
    envelope_path = output_root / "release/gate-d-pre-root-bootstrap-envelope-phase5.50-v1.json"
...
def main() -> None:
    ...
    if args.check:
        ...
        print("Phase 5.50 control-set deterministic generation: PASS")
    else:
        print(f"generated {len(generate(ROOT))} Phase 5.50 control documents")
