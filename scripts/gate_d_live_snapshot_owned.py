...
def capture(*,root:pathlib.Path,command,physical:dict,terminal_path:str,expected_owner_uid:int=0,expected_owner_gid:int=0)->dict:
 ...
 ledger=json.loads(ledger_path.read_text()); owned=ledger.get("ownedFiles"); replaced=ledger.get("replacedFiles")
 if ledger.get("status")!="complete" or ledger.get("checkpoint")!="commit-state" or ledger.get("recoveryRequired") is not False or ledger.get("liveOutput") is not False or not isinstance(ledger.get("release"),str): raise ValueError("administrator ledger is not terminal complete")
 if not isinstance(owned,list) or not isinstance(replaced,list) or not owned+replaced: raise ValueError("administrator ledger lacks committed package inventory")
 records=[]
 for record in owned+replaced:
  if record in replaced and record.get("status")!="committed": raise ValueError("uncommitted package transition")
  if not isinstance(record.get("path"),str): raise ValueError("invalid package path")
  ...
  if path.is_symlink():
   target=os.readlink(path); expected=record.get("symlink",record.get("successorTarget"))
   if target!=expected: raise ValueError("installed symlink differs from ledger")
   records.append({**common,"type":"symlink","target":target})
  elif path.is_file():
   actual=sha(path); expected=record.get("sha256",record.get("successorSha256"))
   if actual!=expected: raise ValueError("installed file differs from ledger")
   records.append({**common,"type":"file","sha256":actual})
 ...
 if terminal_value.get("status")!="complete" or terminal_value.get("recoveryRequired") is not False or terminal_value.get("liveOutput") is not False: raise ValueError("terminal recovery differs")
 ...
 return {"SPDX-License-Identifier":"MIT","schemaVersion":1,"kind":"gate-d-live-target-snapshot","host":{"hostname":command(["hostname"]),"architecture":command(["uname","-m"])},"boot":{"bootId":rooted(root,"/proc/sys/kernel/random/boot_id").read_text().strip()},"kernel":{"release":kernel,"headerPath":header,"headerOwnerUid":hs.st_uid,"headerGroupGid":hs.st_gid,"headerMode":f"0{stat.S_IMODE(hs.st_mode):03o}","configSha256":sha(config),"moduleSymversSha256":sha(header_path/"Module.symvers"),"compiler":command(["cc","--version"]).splitlines()[0]},"signingPolicy":{"source":"config-disabled-sysctl-absent","enforced":False,"sysctl":None,"commandLineEnforced":False,"lockdown":lockdown_value},"packagePaths":records,"packagePathsSha256":canonical_digest(records),"administratorLedger":{"path":LEDGER,"sha256":sha(ledger_path),"ownerUid":st.st_uid,"groupGid":st.st_gid,"mode":"0600","status":ledger.get("status"),"release":ledger.get("release"),"checkpoint":ledger.get("checkpoint"),"recoveryRequired":ledger.get("recoveryRequired"),"liveOutput":ledger.get("liveOutput")},"terminalRecovery":{"path":terminal_path,"sha256":sha(terminal),"ownerUid":terminal_stat.st_uid,"groupGid":terminal_stat.st_gid,"mode":f"0{stat.S_IMODE(terminal_stat.st_mode):03o}","status":terminal_value["status"],"recoveryRequired":False,"liveOutput":False},"runtime":{"moduleLoaded":rooted(root,"/sys/module/rp1_gpclk_dkms").exists(),"endpointPresent":rooted(root,"/dev/rp1-gpclk").exists(),"overlayActive":"rp1-gpclk" in overlays,"dkmsTestVersions":"rp1-gpclk-dkms/" in dkms,"liveOutput":False},"services":services,"physicalSafety":physical}
def main()->None:
