...
def validate(value:dict)->dict:
 ...
 if set(ledger)!={"path","sha256","ownerUid","groupGid","mode","status","release","checkpoint","recoveryRequired","outputActive"} or ledger.get("path")!="/var/lib/rp1-gpclk-dkms/transaction.json" or not SHA.fullmatch(ledger.get("sha256","")) or ledger.get("mode")!="0600" or ledger.get("ownerUid")!=0 or ledger.get("groupGid")!=0 or ledger.get("status")!="complete" or ledger.get("checkpoint")!="commit-state" or ledger.get("recoveryRequired") is not False or ledger.get("outputActive") is not False: raise ValueError("snapshot administrator ledger differs")
 terminal=value["terminalRecovery"]
 if set(terminal)!={"path","sha256","ownerUid","groupGid","mode","status","recoveryRequired","outputActive"} or terminal.get("ownerUid")!=0 or terminal.get("groupGid")!=0 or terminal.get("mode")!="0400" or terminal.get("status")!="complete" or terminal.get("recoveryRequired") is not False or terminal.get("outputActive") is not False or not SHA.fullmatch(terminal.get("sha256","")): raise ValueError("snapshot terminal recovery differs")
 kernel=value["kernel"]
 ...
 if (set(runtime)!={"moduleLoaded","endpointPresent","overlayActive","dkmsTestVersions","outputActive"} or
     any(type(runtime[key]) is not bool for key in runtime) or
     any(runtime[key] for key in ("moduleLoaded","endpointPresent","overlayActive","outputActive"))): raise ValueError("snapshot runtime is active")
