...
"""Deterministically generate the snapshot-bound Phase 5.48 Gate D controls."""
...
import generate_phase5_47_control_set as predecessor_generator

ROOT = pathlib.Path(__file__).resolve().parents[1]
OLD_RELEASE = "0.0.0-phase5.47"
RELEASE = "0.0.0-phase5.48"
OLD_COMMIT = "c5320ac5419a04d17345370204524f219b7ff403"
COMMIT = "ef96f246b66b25bb70536341b60a5f1e64708c65"
OLD_SUFFIX = "phase5.47-c5320ac5419a"
SUFFIX = "phase5.48-ef96f246b66b"
NAMESPACE = SUFFIX
SNAPSHOT = ROOT / "docs/evidence/gate-d-live-target-snapshot-wspr5-phase5.48-v1.json"
BUILD = ROOT / "release/gate-c-representative-build-manifest-phase5.48-v1.json"
...
def generate(output_root: pathlib.Path) -> list[pathlib.Path]:
    ...
    if file_sha(SNAPSHOT) != "9377d109235305f29e85134026cf9247e5d462b0bd2c8e05d9c0463162156e33":
        raise ValueError("Phase 5.48 canonical snapshot differs")
    build = json.loads(BUILD.read_text())
    inventory = json.loads(INVENTORY.read_text())
    artifacts = {item["name"]: item for item in inventory["artifacts"]}
    old_inventory = json.loads((ROOT / "docs/evidence/gate-c-phase5.47-release-input-inventory.json").read_text())
    old_artifacts = {item["name"]: item for item in old_inventory["artifacts"]}
    replacements = {
        OLD_SUFFIX: SUFFIX, OLD_RELEASE: RELEASE, "phase5.47": "phase5.48",
        OLD_COMMIT: COMMIT,
        "497368ac11b32e5491dab76103ad3bb2da0975c086e9898a801c5c2df1be82be": build["candidate"]["archiveSha256"],
        "5c585105cfd5e11a83797bf34f52e9d0ed5a19c5f3ecf7bb74d771f01419ead3": build["result"]["moduleSha256"],
        file_sha(ROOT / "release/gate-c-representative-build-manifest-phase5.47-v1.json"): file_sha(BUILD),
        "7f018fb331c769c44eb1691fdabc4a8a6ff22e0a3debc9e1ed69404d829332b0": file_sha(SNAPSHOT),
        **source_replacements(),
    }
    for old_name, old_item in old_artifacts.items():
        new_name = old_name.replace("phase5.47", "phase5.48")
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for name in CONTROL_NAMES:
            old_path = old_root / f"release/{name}-phase5.47-v1.json"
    ...
    for entry in route["routes"]:
        entry["reason"] = ("Exact Phase 5.48 representative build and canonical inactive "
    ...
    identity_rel = "docs/evidence/gate-d-phase5.48-qualification-install-identity.json"
    identity_path = output_root / identity_rel
    identity_path.parent.mkdir(parents=True, exist_ok=True)
    identity_path.write_bytes(pretty(identity))

    inventory_rel = "docs/evidence/gate-d-phase5.48-predecessor-package-inventory.json"
    ...
    route_rel = "release/gate-d-route-compatibility-decision-phase5.48-v1.json"
    route_path = output_root / route_rel
    route_path.parent.mkdir(parents=True, exist_ok=True)
    route_path.write_bytes(pretty(route))

    bootstrap_rel = "release/gate-d-qualification-bootstrap-plan-phase5.48-v1.json"
    ...
    plan_path = output_root / "release/gate-d-target-operation-plan-phase5.48-v1.json"
    ...
    instance["authorization"].update(
        ...
        approvalScope=("Operator-authorized exact Phase 5.48 output-disabled Gate D scope "
                       "committed at 74e8d1cc9de118e96444ef71c7d0ed34eb25e3d8; limited to "
                       "the 38 indexed namespaced attempts, ten ready rows, exact snapshot, "
                       "snapshot-bound service pre-states, release inputs, predecessor and "
                       "successor inventories, authenticated schema-5 transition, sealed-root "
                       "policy and module graph, recovery, and mandatory prohibitions."))
    ...
    instance["executionPolicy"].update(
        ...
        targetPlan="release/gate-d-target-operation-plan-phase5.48-v1.json",
        ...
        attemptIndex="release/gate-d-attempts-phase5.48-v1/index.json")
    for row in instance["rows"]:
        row["evidenceDirectory"] = f"gate-d/runs/{NAMESPACE}/{row['id']}"

    attempts = gate_d_attempts.generate(instance, plan)
    attempt_dir = output_root / "release/gate-d-attempts-phase5.48-v1"
    ...
    instance_path = output_root / "release/gate-d-execution-instance-phase5.48-v1.json"
    ...
    envelope["priorTerminalState"] = {
        ...
        "mode": ledger["mode"],
        "archivePath": "/var/lib/rp1-gpclk-dkms/recovery/phase5.48-phase5.47-transaction-complete.json",
        "archiveMode": "0400"}
    roles = {"rp1-gpclk-dkms-0.0.0-phase5.48.tar.gz": "archive",
    ...
    envelope_path = output_root / "release/gate-d-pre-root-bootstrap-envelope-phase5.48-v1.json"
...
def main() -> None:
    ...
    if args.check:
        ...
        print("Phase 5.48 control-set deterministic generation: PASS")
    else:
        print(f"generated {len(generate(ROOT))} Phase 5.48 control documents")
